	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (count >= 10 && count < 100) {
			out.println("현재까지의 페이지뷰수[요청횟수] "
					+ "<img src ='images/"+Integer.toString(count).charAt(0)+".png'>"
					+ "<img src ='images/"+Integer.toString(count).charAt(1)+".png'> 번입니다");
		} else if (count >= 100 && count < 1000) {
			out.println("현재까지의 페이지뷰수[요청횟수] "
					+ "<img src ='images/"+Integer.toString(count).charAt(0)+".png'>"
					+ "<img src ='images/"+Integer.toString(count).charAt(1)+".png'>"
					+ "<img src ='images/"+Integer.toString(count).charAt(2)+".png'> 번입니다");
		} else if (count >= 1000 && count < 10000) {
			out.println("현재까지의 페이지뷰수[요청횟수] "
					+ "<img src ='images/"+Integer.toString(count).charAt(0)+".png'>"
					+ "<img src ='images/"+Integer.toString(count).charAt(1)+".png'>"
					+ "<img src ='images/"+Integer.toString(count).charAt(2)+".png'>"
					+ "<img src ='images/"+Integer.toString(count).charAt(3)+".png'> 번입니다");
		} 
		else {
			out.println("현재까지의 페이지뷰수[요청횟수] <img src ='images/"+Integer.toString(count)+".png'> 번입니다");
		}
		out.println("	</center>");
		out.println("</body>");
		out.println("</html>");
